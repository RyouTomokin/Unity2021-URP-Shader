Shader "KIIF/Effect/Transition"
{
    Properties
    {
        [MainColor][HDR] _BaseColor("Color", Color) = (0,0,0,1)
        [MainTexture] _BaseMap("Albedo", 2D) = "white" {}
        _BaseMapAsDissolve("颜色图只作为溶解", Range(0, 1)) = 1
        _Dissolve("溶解进度(UV0.z)", Range(0.0, 1.0)) = 0
...
//        _MoveToCamera("移向摄像机", Range(-20 , 20)) = 0
        ...
        [Enum(UnityEngine.Rendering.BlendMode)] _AlphaBlendMode("AlphaBlendMode(One)", Float) = 1
...
//        [HideInInspector] _ZWrite("__zw", Float) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Blend SrcAlpha OneMinusSrcAlpha , [_AlphaBlendMode] OneMinusSrcAlpha
...
//            ZTest [_ZTest]
            ...
            half4 frag (VaryingsParticle input) : SV_Target
            {
                ...
                // 基础功能
                half4 ColorMap = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv);
                
                half4 color = lerp(ColorMap * _BaseColor, _BaseColor, _BaseMapAsDissolve);
                color *= vertexColor;

                //溶解
                half dissolveFactor = input.texcoord.z + _Dissolve;
                color.a *= Dissolve(dissolveFactor, ColorMap.r).x;
